...
describe ReportBuilder do
  before(:each) do
    @assessment =
      create(
        :tagged_animal_assessment,
        shl_case_number: 'SF16-9A', measurement_date: '09/20/18'
      )
    create(
      :tagged_animal_assessment,
      shl_case_number: 'SF16-9B', measurement_date: '06/20/18'
    )
    create(
      :tagged_animal_assessment,
      shl_case_number: 'SF16-9C', measurement_date: '09/20/18'
    )
    create(
      :tagged_animal_assessment,
      shl_case_number: 'SF16-9A', measurement_date: '03/20/18'
    )
  end
  ...
  describe '#measurement_date_options' do
    context 'when given no cohort' do
      it 'returns all animal assessment measurement dates' do
        ...
        expect(
          @report_builder.measurement_date_options.map(&:to_s)
        ).to include '2018-09-20'
      end
    end
  ...
  describe '#processed_file_id' do
    context 'when given a date and cohort' do
      it 'returns the corresponding animal assessments' do
        @report_builder =
          ReportBuilder.new(cohort: 'SF16-9A', date: '2018-09-20')
