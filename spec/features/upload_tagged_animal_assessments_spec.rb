...
describe 'upload TaggedAnimalAssessment category', type: :feature do
  ...
  let(:incomplete_data_file) do
    "#{Rails.root}/spec/support/csv/Tagged_assessment_03172018-invalid-rows.csv"
  end
  let(:expected_success_message) do
    'Successfully queued spreadsheet for import'
  end
  let(:temporary_file) do
    create(:temporary_file, contents: File.read(valid_file))
  end
  ...
  context 'when user uploads a CSV with invalid headers' do
    it "creates new ProcessedFile record with 'Failed' status" do
      ...
      expect(
        processed_file.job_errors
      ).to eq 'Does not have valid header(s). Data not imported!'
      expect(processed_file.job_stats).to eq({})
      expect(page).to have_content expected_success_message
    end
  end
  ...
  context 'when user upload file with invalid rows' do
    it "creates new ProcessedFile record with 'Failed' status" do
      ...
      expect(ProcessedFile.count).to eq 1
      expect(processed_file.status).to eq 'Failed'
      expect(processed_file.job_errors).to eq(
        'Does not have valid row(s). Data not imported!'
      )
      expect(processed_file.job_stats).to eq(
        {
          'row_number_2' => { 'shl_case_number' => [{ 'error' => 'blank' }] },
          'row_number_3' => { 'tag' => [{ 'error' => 'blank' }] }
        }
      )
      expect(page).to have_content expected_success_message
    end
