...
shared_examples_for 'import job' do
  let(:local_sample_data_filepath) do
    Rails.root.join(
      'db',
      'sample_data_files',
      described_class.category.underscore,
      filename
    )
  end
  let(:sample_csv_text) do
    File.read(local_sample_data_filepath, encoding: 'bom|utf-8')
  end
  ...
  describe '#validate_headers' do
    ...
    it 'returns false for invalid headers' do
      invalid_headers_sample_data_filepath =
        Rails.root.join('spec', 'support', 'csv', 'invalid_headers.csv')
      invalid_headers_sample_csv_text =
        File.read(invalid_headers_sample_data_filepath, encoding: 'bom|utf-8')
      invalid_headers_temporary_file =
        create(:temporary_file, contents: invalid_headers_sample_csv_text)
      validate_headers =
        described_class.new.validate_headers(invalid_headers_temporary_file)
  ...
  it 'imports record into db' do
    ...
    expect { instance.perform(temporary_file, filename) }.to change {
      instance.category.constantize.count
    }

    record_count_after = instance.category.constantize.count
    expect(record_count_before < record_count_after).to eq(true)
  end
end
