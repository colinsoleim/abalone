...
class PopulationEstimate < ApplicationRecord
  ...
  validates :sample_date,
            :shl_case_number,
            :abundance,
            :facility,
            presence: true
  validates :abundance, numericality: true
  validate :facility_is_valid, :lifestage_is_valid
  ...
  def self.create_from_csv_data(attrs)
    ...
    attrs[:sample_date] =
      begin
        Date.strptime(attrs[:sample_date], '%m/%d/%y')
      rescue StandardError
        nil
      end
    attrs[:spawning_date] =
      begin
        Date.strptime(attrs[:spawning_date], '%m/%d/%y')
      rescue StandardError
        nil
      end
  ...
  def facility_is_valid
    ...
    errors.add(
      ...
      "#{facility} does not match the code for any known facility"
    )
  end
