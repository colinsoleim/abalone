...
class WildCollection < ApplicationRecord
  ...
  HEADERS = {
    ...
    FINAL_HOLDING_FACILITY_AND_DATE_OF_ARRIVAL:
      'Final holding facility & date of arrival',
  ...
  validates :gonad_score,
            format: { with: /\A(?:(?:NA)|(?:[0-3](?:-[13])?\??))/ },
            allow_blank: true
  ...
  def self.create_from_csv_data(attrs)
    attrs[:proximity_to_nearest_neighbor] =
      attrs.delete(:proximity_to_nearest_neighbor_m)
    attrs[:collection_depth] = attrs.delete(:collection_depth_m)
    attrs[:collection_coordinates] = attrs.delete(:collection_coodinates)
    attrs[:final_holding_facility_and_date_of_arrival] =
      attrs.delete(:final_holding_facility_date_of_arrival)
    attrs[:collection_date] =
      begin
        Date.strptime(attrs[:collection_date], '%m/%d/%y')
      rescue StandardError
        nil
      end
    attrs[:otc_treatment_completion_date] =
      begin
        Date.strptime(attrs[:otc_treatment_completion_date], '%m/%d/%y')
      rescue StandardError
        nil
      end
  ...
  def initial_holding_facility_is_valid
    ...
    errors.add(
      :initial_holding_facility,
      "#{
        initial_holding_facility
      } does not match the code for any known facility"
    )
  end
  ...
  def final_facility_and_date_of_arrival_is_valid
    ...
    matches =
      /\A(?<facility>[A-Za-z ]+);\s*(?<date>.+)\z/.match(
        final_holding_facility_and_date_of_arrival
      )
    ...
    if matches.nil?
      errors.add(
        :final_holding_facility_and_date_of_arrival,
        "could not parse #{final_holding_facility_and_date_of_arrival}"
      )
    ...
    unless Facility.valid_code?(matches[:facility])
      errors.add(
        :final_holding_facility_and_date_of_arrival,
        "#{matches[:facility]} does not match the code for any known facility"
      )
    end
    ...
    errors.add(
      :final_holding_facility_and_date_of_arrival,
      "#{matches[:date]} could not be parsed as a valid date"
    )
  end
