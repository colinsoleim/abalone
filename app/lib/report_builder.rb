class ReportBuilder
  ...
  def build_cohort_options(date:)
    if date
      find_animal_assessments(date: date, cohort: nil).pluck(:shl_case_number)
        .uniq
    else
  ...
  def build_date_options(cohort:)
    if cohort
      find_animal_assessments(cohort: cohort, date: nil).pluck(
        :measurement_date
      ).uniq
    else
  ...
  def find_animal_assessments(cohort:, date:)
    if cohort and date and
         TaggedAnimalAssessment.exists?(
           shl_case_number: cohort, measurement_date: date
         )
      TaggedAnimalAssessment.where(
        shl_case_number: cohort, measurement_date: date
      )
